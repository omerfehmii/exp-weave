...
- `observed_only: false`
- `future_obs_mode: nearest` for real‑time horizon testing (exact 24h is often coverage‑limited).
...
- Policy config must be `configs/backtest_return_dynCap2.yaml` (DynCap‑2 spec).
...
- `configs/CS_L1_W10_OOS_REAL24_EXACT.yaml` (coverage usually weak; use only for feasibility checks)
...
- `summary_all.json`:
  - aggregate mean/std/sharpe + coverage aggregates
- `wf_summary.json`:
  - `coverage_summary`, `time_units.effective_horizon_hours`
  - `overall_active_coverage.time_ge_min_ic_ratio`
  - `coverage_warn_folds`
- Per‑fold:
  - `coverage.csv` (active_count, ic_count)
  - `metrics.csv` (Sharpe, Sharpe_gross, turnover, dominance)
  - `protocol.json`
- Nearest mode adds:
  - `nearest_gap_p50/p90/mean` in coverage stats (how far forward labels drift).
