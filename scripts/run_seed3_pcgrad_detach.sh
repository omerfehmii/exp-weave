...
if [ ! -f "${SUMMARY}" ]; then
  echo "run_name,model,seed,config,checkpoint,forecast_mae,forecast_pinball,forecast_coverage90,forecast_width90,aci_mae,aci_coverage90,aci_width90,dir_wMCC,dir_wAUC,dir_acc_mean,dir_mcc_mean,dir_auc_mean,dir_tp_sum,dir_tn_sum,dir_fp_sum,dir_fn_sum,bin_max_acc,bin_max_mcc,bin_max_auc,boot_h1_acc,boot_h1_acc_lo,boot_h1_acc_hi,boot_h1_mcc,boot_h1_mcc_lo,boot_h1_mcc_hi,boot_h24_acc,boot_h24_acc_lo,boot_h24_acc_hi,boot_h24_mcc,boot_h24_mcc_lo,boot_h24_mcc_hi" > "${SUMMARY}"
fi
...
for entry in "${RUNS[@]}"; do
  ...
  RUN_NAME="${RUN_NAME}" MODEL_NAME="${MODEL_NAME}" SEED="${SEED}" CFG_PATH="${CFG_PATH}" CKPT_PATH="${CKPT_PATH}" OUT_DIR="${OUT_DIR}" SUMMARY="${SUMMARY}" ${PY} - <<'PY'
import csv
import os
import numpy as np
